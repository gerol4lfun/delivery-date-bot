...
var DIRECTION_ALIAS = {
  'москва и мо': 'Москва и МО', 'москва и м.о.': 'Москва и МО', 'москва': 'Москва и МО', 'мск': 'Москва и МО', 'msk': 'Москва и МО', 'московская область': 'Москва и МО',
  'санкт-петербург и обл.': 'Санкт-Петербург', 'санкт-петербург и ло': 'Санкт-Петербург',
  'спб и ло': 'Санкт-Петербург', 'питер': 'Санкт-Петербург', 'петербург': 'Санкт-Петербург', 'спб': 'Санкт-Петербург', 'ленинградская область': 'Санкт-Петербург',
  'белгород': 'Белгород', 'белгородская область': 'Белгород',
  'великий новгород': 'Великий Новгород', 'новгородская область': 'Великий Новгород',
  'владимир': 'Владимир', 'владимирская область': 'Владимир',
  'вологда': 'Вологда', 'вологодская область': 'Вологда', 'вологодская обл.': 'Вологда',
  'воронеж': 'Воронеж', 'воронежская область': 'Воронеж',
  'екатеринбург': 'Екатеринбург', 'свердловская область': 'Екатеринбург', 'свердловская обл.': 'Екатеринбург',
  'иваново': 'Иваново', 'ивановская область': 'Иваново', 'ивановская обл.': 'Иваново',
  'йошкар-ола': 'Йошкар-Ола', 'марий эл': 'Йошкар-Ола', 'республика марий эл': 'Йошкар-Ола',
  'казань': 'Казань',
  'калуга': 'Калуга', 'калужская область': 'Калуга',
  'кемерово': 'Кемерово', 'кемеровская область': 'Кемерово', 'кузбасс': 'Кемерово',
  'кострома': 'Кострома', 'костромская область': 'Кострома', 'костромская обл.': 'Кострома',
  'краснодар': 'Краснодар', 'краснодарский край': 'Краснодар', 'кубань': 'Краснодар',
  'курск': 'Курск', 'курская область': 'Курск',
  'липецк': 'Липецк', 'липецкая область': 'Липецк',
  'майкоп': 'Майкоп', 'адыгея': 'Майкоп', 'республика адыгея': 'Майкоп',
  'набережные челны': 'Набережные Челны', 'челны': 'Набережные Челны',
  'нижний новгород': 'Нижний Новгород', 'нн': 'Нижний Новгород', 'нижний': 'Нижний Новгород', 'нижегородская область': 'Нижний Новгород', 'нижегородская обл.': 'Нижний Новгород',
  'новосибирск': 'Новосибирск', 'новосибирская область': 'Новосибирск',
  'орел': 'Орел', 'орёл': 'Орел', 'орловская область': 'Орел',
  'рязань': 'Рязань', 'рязанская область': 'Рязань',
  'ставрополь': 'Ставрополь', 'ставропольский край': 'Ставрополь',
  'тамбов': 'Тамбов', 'тамбовская область': 'Тамбов',
  'тверь': 'Тверь', 'тверская область': 'Тверь',
  'тула': 'Тула', 'тульская область': 'Тула',
  'ульяновск': 'Ульяновск', 'ульяновская область': 'Ульяновск',
  'чебоксары': 'Чебоксары', 'чувашия': 'Чебоксары', 'республика чувашия': 'Чебоксары',
  'челябинск': 'Челябинск', 'челябинская область': 'Челябинск', 'челябинская обл.': 'Челябинск',
  'черкесск': 'Черкесск', 'карачай-черкесия': 'Черкесск', 'карачаево-черкесская республика': 'Черкесск',
  'ярославль': 'Ярославль', 'ярославская область': 'Ярославль', 'ярославская обл.': 'Ярославль', 'ярославкая обл.': 'Ярославль',
...
function processSheetRows(rows) {
  ...
  while (i < rows.length) {
    ...
    for (var mi = 0; mi < MONTH_NAMES.length; mi++) {
      if (MONTH_NAMES[mi] === monthInfo.month) {
        monthNum = mi + 1;
        break;
      }
    }
    ...
    for (var j = 1; j < row.length; j++) {
      var d = extractDayNumber(row[j]);
      if (d != null) {
        dayNumbers.push(d);
      } else if (dayNumbers.length > 0) break;
    }

    if (dayNumbers.length === 0 || monthNum == null) { i++; continue; }
...
function runDeliverySync() {
  ...
  Logger.log('delivery-sync: rows=' + rows.length + ' blocks=' + allBlocks.length + ' ' + allBlocks.map(function(b){ return b.year + '-' + (b.monthNum < 10 ? '0' : '') + b.monthNum + '@' + b.dayNumbers.length; }).join(', ') + ' dry_run=' + dryRun);
  var options = {
    method: 'post',
